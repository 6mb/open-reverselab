...
MIN_TAG_COUNT="${MIN_TAG_COUNT:-0}"
MIN_RELEASE_COUNT="${MIN_RELEASE_COUNT:-0}"
...
while IFS=$'\t' read -r branch_ref branch_sha; do
  ...
  if [ "$branch_name" = "$CONTROL_BRANCH" ] || [[ "$branch_name" == "$BACKUP_PREFIX/"* ]]; then
    continue
  fi
...
while IFS= read -r row; do
  ...
  if gh release view "$tag" --repo "$TARGET_REPO" >/dev/null 2>&1; then
    ...
    create_args=(release create "$tag" --repo "$TARGET_REPO" --title "$name" --notes-file "$body_file" --verify-tag)
  ...
  jq -c '.assets[]?' <<<"$target_release_json" | while IFS= read -r target_asset_json; do
    ...
    grep -Fxq "$target_asset_name" "$source_asset_names" && continue
